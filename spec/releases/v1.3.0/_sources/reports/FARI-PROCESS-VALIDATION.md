...
2. **Optional mappings added useful context.** The reports corrected or refined
   external references without changing the supplied technical evidence.
...
report may still contain remediated or still-open findings.

**Change made:** Added Draft, Provisional, Final, and Superseded report-maturity
states to the framework contract. The current web lifecycle exposes
Provisional and Final through its open/closed workflow while preserving the
full maturity set for normalized records and future revisions.

### PV-006: Supplied Mappings Require Validation

Both cases proposed a broad external classification, but the supplied evidence
was more accurately related to software code flaws and command behavior. Mapping
by title alone would have reduced report quality.
...
### PV-015: External Mappings Need Evidence-Level Precision

The SpaceCAN source material proposed several related external references. The
local replay file demonstrates transmission, but it does not demonstrate uplink
interception or broader operational effects.
...
flowchart LR
    ...
    R["Relate<br/>findings + mappings + mission risk"]
...
inconclusive outcomes, mapping correction, and consolidation. They do not yet
validate:
...
- Advanced evidence-producer handoff workflows beyond the current web intake.
